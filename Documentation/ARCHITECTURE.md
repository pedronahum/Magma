...
**Purpose**: x10-style lazy evaluation with graph tracing, optimization passes, and compilation caching
...
├── Optimization/
│   ├── PassManager.swift         # Optimization pass infrastructure
│   ├── DeadCodeElimination.swift # Remove unused operations
│   ├── CommonSubexpressionElimination.swift  # Eliminate redundant computations
│   ├── ConstantFolding.swift     # Evaluate constant expressions at compile time
│   ├── AlgebraicSimplification.swift  # Simplify algebraic patterns (x+0=x, x*1=x, etc.)
│   ├── FusionPatterns.swift      # Pattern matching infrastructure for fused ops
│   └── Patterns/
│       ├── ScaledDotProductAttentionPattern.swift  # Q·K^T/√d · V fusion
│       ├── LayerNormPattern.swift     # Layer normalization fusion
│       ├── RMSNormPattern.swift       # RMS normalization fusion
│       ├── MatMulBiasActivationPattern.swift  # MatMul + bias + activation fusion
│       ├── SoftmaxPattern.swift       # Softmax pattern (exp-sum-div)
│       └── GeluPattern.swift          # GELU activation pattern
├── Execution/
...
public enum OpKind: String {
    ...
    // Fused operations (generated by optimization passes)
    case fusedScaledDotProductAttention
    case fusedLayerNorm, fusedRMSNorm
    case fusedMatMulBiasActivation
    case fusedSoftmax, fusedGelu
    // ... complete op set
}
