...
| **1. DDP MVP (single host)** | ✅ `DeviceMesh` (#11); ✅ `all_reduce` op + cross-replica runtime (#12); ✅ `allReduceMean` gradient-sync primitive + param-sync invariant verified across replicas (#13); ⬜ `DistributedSampler` (#14); ⬜ DDP end-to-end with a real model (#15) | **M–L** |
