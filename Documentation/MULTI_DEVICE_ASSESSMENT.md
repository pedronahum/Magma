...
| **3. FSDP / tensor parallel** | ✅ tensor parallelism via Shardy — a contracting-dim-sharded matmul makes Shardy insert an all_reduce; each device holds the full result == single-device reference (#21). Full FSDP (sharded params all-gathered on use + grads reduce-scattered across a training step) builds on the same partitioner — the collective-insertion mechanism is proven | **L** |
| **4. Multi-host** | ✅ testable slice — `MultiHostConfig` (process/device topology, global device ids) + global data sharding across a cluster via `DistributedSampler.multiHost` (globally disjoint & covering, verified) (#22). ⬜ The coordination service itself (PJRT distributed init: gRPC coordinator + KV rendezvous, NCCL id exchange) needs a real multi-host runtime — not testable on one box; `MultiHostConfig` is the config it consumes | **L** |
