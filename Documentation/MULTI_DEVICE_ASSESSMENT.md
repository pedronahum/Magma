...
| **1. DDP MVP (single host)** | ✅ `DeviceMesh` (#11); ✅ `all_reduce` op + cross-replica runtime (#12); ✅ `allReduceMean` gradient-sync primitive + param-sync invariant (#13); ✅ `DistributedSampler` + per-replica batching, integrated into `DataLoader` (#14); ⬜ DDP end-to-end with a real model (#15) | **M–L** |
