...
| **2. SPMD / Shardy** | ✅ pure-Swift sdy types (#16); ✅ MLIRBuilder sdy hooks (#17); ✅ Shardy compile flags (#18); ✅ track sharding on graph nodes — `LazyTensorHandle.sharding` + `IRGraph.mesh` threaded through the emitter (`sdy.mesh` + arg `{sdy.sharding}` + intermediate `sdy.sharding_constraint`) with `validateShardings` (#19); ⬜ SPMD end-to-end partition→execute run (#20). **New vs SwiftIR:** the partition→collectives→multi-device-execute closure SwiftIR never did | **L** |
